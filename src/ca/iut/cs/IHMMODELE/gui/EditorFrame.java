...
import ca.iut.cs.IHMMODELE.application.ExtensionFilter;
import ca.iut.cs.IHMMODELE.application.RecentFilesQueue;
...
import ca.iut.cs.IHMMODELE.persistence.DeserializationException;
import ca.iut.cs.IHMMODELE.persistence.PersistenceService;
...
public class EditorFrame extends JFrame
{
	...
	{  
		...
		aEditorResources = ResourceBundle.getBundle("ca.iut.cs.IHMMODELE.gui.EditorStrings");      
		MenuFactory factory = new MenuFactory(aEditorResources);
		
		aRecentFiles.deserialize(Preferences.userNodeForPackage(FormEditor.class).get("recent", "").trim());
   ...
   {  
   	...
   	private String setTitle(JInternalFrame pInternalFrame)
   	{
   		...
   		else
   		{
   			...
   			if( file == null )
   			{
   				...
   				if(graphType instanceof FormDiagramGraph)
   				{
   					diagramName = "Nouveau Modèle";
   				}
   				return diagramName;
   			}
   			else
   	...
   	public void addWelcomeTab()
   	{
     	...
     	aTabbedPane.add("Bienvenue", aWelcomeTab);
     ...
     * Asks the user to open a graph file.
   	...
   	{  
		...
		ExtensionFilter[] filters = new ExtensionFilter[]{
			new ExtensionFilter(aAppResources.getString("Form.name"), 
					aAppResources.getString("Form.extension") + aAppResources.getString("files.extension")),
   		};
   	...
   	public void saveAs()
   	{
   		...
   		try
   		{
   			...
   			if(response == JFileChooser.APPROVE_OPTION)
   			{
   				...
   				{
   	        		ResourceBundle editorResources = ResourceBundle.getBundle("ca.iut.cs.IHMMODELE.gui.EditorStrings");
     ...
     * or if the user agrees to abandon modified graphs.
   	...
   	public void exit()
   	{
   		...
   		Preferences.userNodeForPackage(FormEditor.class).put("recent", aRecentFiles.serialize());
   		System.exit(0);
   	}
}
